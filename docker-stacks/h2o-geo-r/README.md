# Jupyter Notebook Geo Data Science with H20 Stack
...
```
docker run -it --rm -p 8888:8888 nijmegenr/h2o-geo-r
```
...
```
docker run -d -p 8888:8888 nijmegen/h2o-geo-r start-notebook.sh --NotebookApp.password='sha1:74ba40f8a388:c913541b7ee99d15d5ed31d4226bf7838f83a50e'
```

Bijvoorbeeld, om de basis URL van de Notebook server te zetten, doe het volgende:

```
docker run -d -p 8888:8888 nijmegen/h2o-geo-r start-notebook.sh --NotebookApp.base_url=/some/path
```

Bijvoorbeeld, om alle authenticatie mechanismen uit te schakelen (niet aanbevolen):

```
docker run -d -p 8888:8888 nijmegenr/h2o-geo-r start-notebook.sh --NotebookApp.token=''
```
...
docker run -d -p 8888:8888 \
    ...
    nijmegenr/h2o-geo-r start-notebook.sh \
...
c.DockerSpawner.container_image = 'nijmegenr/h2o-geo-r'
...
```
docker run -it --rm nijmegenr/h2o-geo-r start.sh ipython
```
